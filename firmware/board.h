// Board and chip pin mapping for a One ROM Fire 24 emulating a 1801RE2.
//
// Two board revisions are supported.  Their socket-to-GPIO maps are identical
// in every signal pin -- the eleven-plus AD lines, the three strobes, CS, the
// unused socket pins and both X pads -- so the serving path does not know
// which board it is on.  What differs is the furniture:
//
//                  rev E                        rev F
//   status         plain LED on GPIO 29         WS2812B pixel on GPIO 29
//   jumper A       GPIO 25   (recovery)         GPIO 26   (recovery)
//   jumper B       GPIO 24                      GPIO 27
//   jumpers C, D   GPIO 26, 27 (= SWD pads)     GPIO 25, 24 (= SWD pads)
//
// Select with -DONEROM_BOARD=FIRE24F from the build; rev E is the default,
// because that is the board this firmware was developed and proven on.
//
// The socket-pin -> GPIO half of this file is taken from the One ROM project's
// own board descriptions (rust/config/json/fire-24-{e,f}.json) and is known
// good.  The chip-pin -> signal half comes from the KR1801RE2 datasheet,
// cross-checked against the MS 0511 schematic; see the RE2 section below.

#ifndef BOARD_H
#define BOARD_H

#ifndef ONEROM_BOARD_FIRE24F
#define ONEROM_BOARD_FIRE24F 0
#endif

#if ONEROM_BOARD_FIRE24F
...
// ---------------------------------------------------------------------------
// 24-pin socket to RP2354A GPIO -- identical on rev E and rev F
// ---------------------------------------------------------------------------
//
