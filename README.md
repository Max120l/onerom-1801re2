...
The board also turned out to be a good instrument. It sits on the bus, sees
every cycle, and is the one device in the machine you know is working — so the
same firmware, built with one flag, becomes a diagnostic tool for the machine
around it. That half is optional, off by default, and compiles to nothing: a
plain build is byte-identical to one from a tree with the diagnostics deleted.
This README is the ROM emulator. The rest is next door:

| | |
|---|---|
| [docs/DIAGNOSTICS.md](docs/DIAGNOSTICS.md) | the instruments: build matrix, how to read the LED, the test ROMs |
...
The repository is split down one line: **serving a ROM**, and **using the same
board as an instrument**. The second is entirely optional, off by default, and
compiles to nothing — a plain build is byte-identical to one from a tree with
the diagnostics deleted.
...
| `tools/rom/gen_rom_images.py` | emits `firmware/rom_images.c` from dumps |
| `tools/rom/diagnose_dump.py` | tells a bad dump from a bad chip |
| `tools/rom/rom_checksum.py` | the machine's own ROM test, run offline |

**The diagnostics** — see [docs/DIAGNOSTICS.md](docs/DIAGNOSTICS.md)

| File | |
|------|--|
| `firmware/diag.c` | scoring, beacons and every LED frame |
| `firmware/diag.h` | what the serving loop calls; empty inlines without `MPI_WATCH` |
| `firmware/watch.h` | the watchpoint and beacon tables |
| `tools/diag/make_*.py` | test ROMs that replace the system monitor |
| `tools/diag/selftest.py` | the self-test pattern, shared by generator and checker |
| `tools/diag/check_selftest.py` | diagnoses a dump of the self-test build |
| `test/test_watchpoints.py` | checks those watchpoints against the ROM |
| `test/test_*test.py` | runs each test ROM in a model of the PP |
...
```console
$ ./tools/rom/re2_convert.py 205_mc0511.rom
205_mc0511.rom: code=3
  as read   RTS PC=   0  JSR PC=   0
  converted RTS PC= 110  JSR PC=  76

$ ./tools/rom/gen_rom_images.py -o firmware/rom_images.c \
...
### The diagnostic builds

Everything about them is in **[docs/DIAGNOSTICS.md](docs/DIAGNOSTICS.md)**: the
build matrix, how to read the LED frame, the watchpoint table, and the four test
ROMs that replace the system monitor to interrogate a machine too broken to
boot. The short version is `-DMPI_WATCH=ON`, and that the board serves the bus
identically either way — the scoring happens after the reply has been queued, so
a watch build cannot introduce the timing fault it might be used to rule out.
...
```console
$ ./tools/rom/gen_rom_images.py --selftest -o firmware/rom_images.c
...
```console
$ ./tools/diag/check_selftest.py dumped.bin --window 3
```
